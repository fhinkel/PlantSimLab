...
int main(int argc, char * argcv[]){
  ...
  int numRandomRuns  = 0;
  bool tables = true, pds = false, writeFile = false, edges = false, verbose = false, traj = false, multitraj = false, randomUpdate= false, includeTables =false;
  int state_check;
  char ** multitrajFiles;
  ...
  if (input.length() > 0){
    ...
    if (edges){
      ...
      if (randomUpdate){
	cyc->generateRandomTrajectory(writeFile, outputFile, verbose, trajFile, filename, numRandomRuns, numRandomSteps, includeTables);
      }
      else {
	cyc->generateTrajectory(writeFile, outputFile, verbose, trajFile, filename, includeTables);
      }
    }
    else if (multitraj){
	...
	if (writeFile){
	  if (randomUpdate){
	    cyc->generateRandomTrajectory(writeFile, multitrajOutfiles[i], verbose, multitrajFiles[i], filename, numRandomRuns, numRandomSteps, includeTables);
	  }
	  else {
	    cyc->generateTrajectory(writeFile, outputFile, verbose, trajFile, filename, includeTables);
	  }
	  ...
	  if (randomUpdate){
	    cyc->generateRandomTrajectory(writeFile, outputFile, verbose, multitrajFiles[i], filename, numRandomRuns, numRandomSteps, includeTables);
	  }
	  else {
	    cyc->generateTrajectory(writeFile, outputFile, verbose, trajFile, filename, includeTables);
	  }
